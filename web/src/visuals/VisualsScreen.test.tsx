...
vi.mock("./VisualExplanationViewer.js", () => ({ VisualExplanationViewer: ({ visual }: any) => <div>viewer-{visual.visualMessageId}</div> }));
...
describe("VisualsScreen", () => {
  it("loads the requested explanation and starts generation through AVA chat", async () => {
    ...
    render(<VisualsScreen initialVisualId={visualFixture.visualMessageId} onCreate={onCreate} />);
    expect(await screen.findByText(`viewer-${visualFixture.visualMessageId}`)).toBeTruthy();
  ...
  it("keeps cached explanations usable when AVA is offline", async () => {
    ...
    expect(screen.getByText(`viewer-${visualFixture.visualMessageId}`)).toBeTruthy();
  });
});
