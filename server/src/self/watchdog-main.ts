...
//   tsx watchdog-main.ts <repoRoot> <knownGood> <healthUrl> <timeoutMs>
...
async function main(): Promise<void> {
  const [, , repoRoot, knownGood, healthUrl, timeoutMsRaw] = process.argv;
  ...
  await decideRollback({
    ...
    rollback: async () => { revertTo(repoRoot, knownGood); },
  });
  process.exit(0);
}
void main();
