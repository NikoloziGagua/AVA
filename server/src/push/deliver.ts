...
export function buildDeliverer(args: DelivererArgs): Deliverer {
  ...
  return {
    async deliverApprovalPush(approval) {
...
// Convenience standalone — call buildDeliverer once and use:
export async function deliverApprovalPush(args: {
  ...
  const d = buildDeliverer({ db: args.db, vapid: args.vapid, send: args.send });
  return d.deliverApprovalPush(args.approval);
}
