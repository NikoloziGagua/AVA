...
export function TubelightNav({ items, activeName, className }: TubelightNavProps) {
  ...
  return (
    <div
      ...
      className={`glass no-scrollbar flex max-w-[calc(100vw-1rem)] items-center gap-0.5 overflow-x-auto rounded-full p-1 sm:gap-1 ${className ?? ""}`}
    >
      {items.map((item, i) => {
        ...
        return (
          <button
            ...
            className="relative flex shrink-0 cursor-pointer items-center gap-2 rounded-full px-2 py-2 text-[13px] font-semibold text-white/70 transition-colors hover:text-white sm:px-4"
          >
