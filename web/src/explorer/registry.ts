...
const capabilities: ExplorerCapability[] = [
  ...
  },
  {
    ...
    description: "Turns repository maps, request paths and branching processes into compact, captioned Mermaid walkthroughs with drill-down scenes.",
    purpose: "Let Sir ask AVA to explain a system visually without relying on giant diagrams, external renderers or unsafe generated HTML.",
    stability: "beta",
    definition: {
      implementation: "implemented",
      sourceReferences: [
        ...
        source("web/src/visuals/VisualExplanationViewer.tsx", "VisualExplanationViewer"),
        source("server/src/visual-explanations/model.test.ts", "validateVisualExplanation", "test"),
        source("web/src/visuals/VisualExplanationViewer.test.tsx", "VisualExplanationViewer", "test"),
    ...
    inputs: [
      { name: "Mermaid topology", description: "A restricted flowchart with stable, explicit node IDs.", required: true, sensitive: true },
      { name: "storyboard", description: "Versioned scenes referencing those node IDs with captions, highlights, transitions and optional interaction cues.", required: true, sensitive: true },
    ],
    outputs: [
      { name: "canonical visual explanation", description: "Versioned Mermaid and storyboard source stored locally with chat or voice lineage.", persistent: true },
      { name: "sandboxed walkthrough", description: "A progressive, keyboard-accessible local presentation with an accessible text fallback." },
    ...
    dependencies: [
      { targetType: "capability", targetId: "conversation.text-turn", relationship: "uses", required: true, description: "AVA creates explanations through the normal agent/tool path and returns an exact viewer reference." },
      { targetType: "capability", targetId: "interface.pwa", relationship: "uses", required: true, description: "The Visuals surface renders the local viewer inside AVA." },
      { targetType: "data-store", targetId: "sqlite.visual-explanations", relationship: "writes-to", required: true, description: "Only validated Mermaid/storyboard source and lineage are canonical; rendered artifacts are not stored." },
    ],
    readiness: readiness(["defined", "available", "healthy", "tested"], { recentSuccess: true }),
    verification: verification(
      ["Every storyboard reference resolves to one declared stable Mermaid ID.", "Each scene renders locally inside a scriptless, no-network sandbox or exposes the complete static fallback.", "SVG and PNG exports are generated from the active sanitized scene without changing canonical state."],
      ["api-response", "tool-result", "unit-test", "artifact"],
      ["Mermaid v1 is intentionally limited to a safe flowchart grammar.", "The visual communicates supplied structure; it does not independently prove the underlying system is accurate."],
    ),
...
const WORKFLOW_BY_CAPABILITY_ID: Readonly<Record<string, ExplorerWorkflow>> = {
  ...
  "interface.visual-explanations": defineWorkflow(
    ...
    "Keep topology canonical in Mermaid, tell the story through stable-ID scenes, and render only the current bounded view.",
    [
      ...
      { key: "topology", parent: "request", name: "Define stable topology", description: "AVA expresses the actual nodes and edges as restricted Mermaid flowchart source with stable explicit IDs.", kind: "operation", toolName: "visual_explanation_create" },
      ...
      { key: "persist", parent: "validate", name: "Persist canonical source", description: "Store sanitized Mermaid, storyboard and lineage; never store generated HTML, SVG or PNG as canonical state.", kind: "operation", producesEvidence: ["artifact", "api-response"] },
      { key: "scene", parent: "persist", name: "Render one scene locally", description: "Project only the selected scene, render bundled Mermaid locally, sanitize the SVG again and place it in a scriptless no-network iframe.", kind: "operation", producesEvidence: ["visual-confirmation"] },
