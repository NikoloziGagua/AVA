...
const capabilities: ExplorerCapability[] = [
  ...
  },
  {
    ...
    purpose: "Let Sir understand and question a system visually without leaving Chat, relying on giant diagrams, external renderers or unsafe generated HTML.",
    stability: "beta",
    definition: {
      implementation: "implemented",
      sourceReferences: [
    ...
    readiness: readiness(["defined", "available", "healthy", "tested"], { recentSuccess: true }),
    verification: verification(
      ["Every relationship and storyboard reference resolves to one declared stable semantic ID.", "The exact immutable revision reloads inline with its assistant message.", "Each scene renders locally as re-sanitized inert SVG or exposes the complete static fallback.", "SVG and PNG exports are generated from the active sanitized scene without changing canonical state."],
      ["api-response", "tool-result", "unit-test", "artifact"],
      ["The v1 semantic model supports flowcharts and Mermaid is the initial renderer only.", "Voice delegated actions do not yet attach visuals to the spoken transcript.", "The visual communicates supplied structure; it does not independently prove the underlying system is accurate."],
    ),
    safety: safety("low", "never", ["Persists a sanitized visual source and may download a disposable SVG or PNG on explicit user action."], {
      ...
      redactions: ["Secret-like text is scrubbed before persistence; renderer input rejects links, directives, HTML, scripts and external URLs."],
      stopConditions: ["Reject undeclared or duplicate stable IDs.", "Reject active Mermaid features or unsafe markup instead of attempting to render them."],
    }),
    runtime: runtime(
...
const WORKFLOW_BY_CAPABILITY_ID: Readonly<Record<string, ExplorerWorkflow>> = {
  ...
  "interface.visual-explanations": defineWorkflow(
    ...
    [
      ...
      { key: "scene", parent: "persist", name: "Render inline scene", description: "Project the selected semantic scene, render bundled Mermaid locally, sanitize SVG again and inject only inert SVG in the native Chat card.", kind: "operation", producesEvidence: ["visual-confirmation"] },
      ...
      { key: "export", parent: "scene", name: "Export current scene", description: "Generate a disposable sanitized SVG or browser-canvas PNG only when explicitly requested.", kind: "result", producesEvidence: ["artifact"] },
