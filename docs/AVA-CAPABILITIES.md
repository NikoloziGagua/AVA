...
| **memory_index_capture / memory_index_search / memory_index_open / memory_index_forget** | Automatically index completed research and meaningfully developed ideas, accept explicit "remember this" capture, and recover source-linked work across chat and voice. Semantic similarity helps discovery; the original conversation range is re-verified and can be opened before detailed use. | Search/open are read-only; capture/forget are low-risk local persistence. Routine, failed and interrupted turns are excluded; compact content is scrubbed before editor, storage and embedding calls. |
...
- **Source-linked research and idea index:** completed research and ideas
  ...
  stores one compact summary in SQLite plus
